...
def novo_usuario():
    ...
    response = bd_query_post_users_register(
        data["name"], data["email"], data["password"]
    )

    access_token = create_access_token(
        identity=novo_usuario.id, expires_delta=timedelta(days=5)
    )
    fresh_token = create_refresh_token(
        identity=novo_usuario.id, expires_delta=timedelta(days=15)
    )
...
def logar_usuario():
    ...
    if response == False:
        ...
        access_token = create_access_token(
            identity=response.id, expires_delta=timedelta(days=5)
        )
        refresh_token = create_refresh_token(
            identity=response.id, expires_delta=timedelta(days=15)
        )
        return {
            "response": response,
            "access_token": access_token,
